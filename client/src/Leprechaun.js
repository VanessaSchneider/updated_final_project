...
function Leprechaun({ user, taskCompleted, setTaskCompleted }) {
  ...
  return (
    <div>
      <h2>
        Welcome to Leprechaun Landing {user ? <>, {user.username}</> : null}!
      </h2>
      {playGame === true ? (
        <div>
      ...
      {playGame === true ? (
        <Game1
          setRidCorrect={setRidCorrect}
          ridCorrect={ridCorrect}
      ...
      {ridCorrect === 0 && playGame === false && task.task2 !== 1 ? (
        <p className="riddle-correct">
          I'll only give you the map if you can get 2 out of 4 riddles correct!
        </p>
      ) : null}
      {playGame === false && task.task2 === 0 ? (
      ...
      <div className="middle-section">
        {ridCorrect > 1 || task.task2 === 1 ? (
          <div className="lep-correct">
            <br></br>{" "}
            <h3>
              I can't believe you got those riddles correct. Okay, I'll give the
              map!
            </h3>
          </div>
        ) : null}
        {ridCorrect > 1 || task.task2 === 1 ? (
          <div className="button-correct">
            <NavLink to="/witch">
              <button onClick={Play} className="the-witches">
                Witch's Woods
              </button>
            </NavLink>
          </div>
