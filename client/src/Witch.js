...
function Witch() {
  ...
  return (
    <>
      <div>
        <h2>
          Welcome to Witch's Woods! {user ? <>, {user.username}</> : null}!
        </h2>
        ...
        {wtalks === 0 && task.task3 === 0 ? (
          <div>
            {" "}
            <p className="riddle-correct">
              {" "}
              {user ? <>{user.username},</> : null} is it? Well, you aren't
              getting past me. I will get you and your little chicken too!{" "}
            </p>
            <button className="the-button" onClick={increment}>
              {" "}
              Say, "I will get past you!"
            </button>{" "}
          </div>
        ) : null}
        {task.task3 === 1 ? (
          <div className="riddle-correct">
            <h3>I can't believe you're so good at witch trivia!</h3>{" "}
            <NavLink to="/goblin">
        ...
        {wtalks === 1 && playGame === false ? (
          <p className="riddle-correct">
            I have gotten really into trivia lately.If you can answer 3
            questions correctly, I will let you go.
          </p>
        ) : null}

        {playGame === false && wtalks === 1 ? (
