...
function RockContainer({ userWins, setUserWins, UserWins, task }) {
  ...
  return (
    <div>
      {wtalks !== 2 && task.task5 !== 1 ? (
        <img src={warlock} className="size" alt="image"></img>
      ) : null}
      {(wtalks === 0) & (task.task5 !== 1) ? (
        <div>
          {" "}
          <h3 className="riddle-correct">
            Well, well, well. You finally made it to my castle. I see you got
            past my friends guarding the path. I also suppose that chicken with
            you wants to take back his Kingdom?{" "}
          </h3>
      ...
      {wtalks === 1 ? (
        <div>
          {" "}
          <h3 className="riddle-correct">
            There is only one fair way to solve this... and it's Rock, Paper,
            Scissors. If you can win 5 times, I will surrender!
          </h3>
      ...
      {submitted === false && wtalks === 2 ? (
        <div>
          {" "}
          Your Wins: {userWins} Warlock Wins: {warlockWins}{" "}
        </div>
      ) : null}
      {wtalks === 2 && submitted == false ? (
        <div>
          ...
          <div>
            ...
            <button className="buttons" value="rock" onClick={handleClick}>
              Rock
            </button>
          </div>
          <div>
            ...
            <button className="buttons" value="paper" onClick={handleClick}>
              Paper
            </button>
          </div>
          <div>
            ...
            <button className="buttons" value="scissors" onClick={handleClick}>
              Scissors
            </button>
          </div>
        </div>
      ...
      {submitted === true ? (
        <div>
          ...
          <br></br>
          {Wins()}
          <button className="buttons" onClick={Submitted}>
            Next Round
          </button>
        </div>
      ) : null}
