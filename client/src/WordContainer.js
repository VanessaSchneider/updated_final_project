...
function WordContainer({
  ...
  let blanksToShow = letters.map((letter) => (
    <div className="catTile" key={letter.id}>
      {letter.letter2}
    </div>
  ));
  ...
  async function handleSubmit(e) {
    ...
    let lettersArr = [];
    lettersArr = letters.map((letter) => letter.letter);

    if (guess && lettersArr.includes(guess)) {
    ...
    <div>
      ...
      {finalCorrect.length === 7 || (user && user.task.task4 === 1) ? (
        <div className="namegame2">
          <NavLink to="/warlock">
            <button onClick={Play} className="buttons">
              Warlock's Castle
            </button>
          </NavLink>
          <h2 className="namegame">
            I can't believe you guessed it! Great name, right? Now I will let
            you cross.
          </h2>
        </div>
      ) : null}
